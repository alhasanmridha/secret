...
import org.jivesoftware.smackx.jingleold.packet.JingleDescription.WebRTCSDP;
...
public class JingleSession extends JingleNegotiator implements MediaReceivedListener {
    ...
    public synchronized void receivePacketAndRespond(IQ iq) throws XMPPException, SmackException, InterruptedException {
        ...
        Log.e("Packet", "=========================================>" + iq.getType().toString());
        if(iq.getType().toString().equals("result")){
//            AudioVideoCall.startBeep();
        }
    ...
    public List<IQ> dispatchIncomingPacket(IQ iq, String id) throws XMPPException, SmackException, InterruptedException {
        ...
        if (iq != null) {
            if (iq.getType().equals(IQ.Type.error)) {
                ...
                Log.e("Packet:", "-------------------------------->" + action.toString() + getSessionState().toString());
            ...
            public boolean accept(Stanza packet) {
                ...
                if (packet instanceof IQ) {
                    ...
                    if (iq instanceof Jingle) {
                        ...
                        for (JingleContent jingleContent : jin.getContentsList()) {
                            if (jingleContent.getName().equals("sdp")) {
                                setSDP(jingleContent.getDescription().getSDP());
                            }else if (jingleContent.getName().equals("candidates")) {
                                addCandidate(jingleContent.getDescription().getCandidate());
                            }
                        }
                    } else {
...
//        jingle.addContent();
        ...
        sendStanza(jingle);
        Log.e("Packet",jingle.getChildElementXML().toString());
...
//        }
    ...
    public void startOutgoing(String sdp) throws IllegalStateException, SmackException, InterruptedException {
        ...
        JingleContent jcon = new JingleContent("initiator","sdp");
        jcon.setDescription(new WebRTCSDP(sdp));


        jingle.addContent(jcon);
        // Save the session-initiate packet ID, so that we can respond to it.
        sessionInitPacketID = jingle.getStanzaId();
    ...
    public void addCandidate(String ncandidate) {candidates.add(ncandidate);}

    public String getSDP() {
        return sdp;
    }

    public void setSDP(String nsdp) {
        sdp = nsdp;
    }
}
